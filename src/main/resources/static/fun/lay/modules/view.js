layui.extend({
...
}).define(['jquery', 'element', 'laytpl','loadBar', 'dropdown'], function (exports) {
    var $ = layui.jquery
        , conf = layui.conf
    ...
    self.loading = function (elem) {
        elem.append(
            (this.elemLoad = $(
                '<i class="layui-anim layui-anim-rotate layui-anim-loop layui-icon layui-icon-loading fun-loading"></i>'
            ))
        )
    };
    ...
    self.tab = {
        ...
        init: function () {
            var tab = this;
            var btnCls = tab.wrap + ' .fun-tabs-btn';
            ...
            layui.dropdown.render({
                ...
                click: function (name) {
                    ...
                    if (name === 'current') {
                        tab.del(layui.fun.route.fileurl);
                    }
                    if (name === 'refresh') {
                        tab.refresh();
                    }
            ...
            $(document).on('click', btnCls, function (e) {
                ...
                if ($(e.target).hasClass('fun-tabs-close')) {
                    tab.del(url)
                } else {
            ...
            $('.fun-tabs-hidden').addClass('layui-show');
            this.isInit = true
        },
        has: function (url) {
            var exists = false;
        ...
        delAll: function (type) {
            var tab = this;
            var menuBtnClas = tab.menu + ' .fun-tabs-btn';
        ...
        del: function (url, backgroundDel) {
            ...
            var lay = '[lay-url="' + url + '"]';
            var thisBody = $(
                '#' + conf.containerBody + ' > .fun-tabs-item' + lay
            );
            ...
            if (backgroundDel === undefined) {
                if (thisMenu.hasClass('fun-tabs-active')) {
                    $(this.menu + ' li:last').click()
                }
            }
        },
        refresh: function (url) {
            url = url || layui.fun.route.fileurl;
            if (this.has(url)) {
                this.del(url, true);
                self.renderTabs(url)
            }
        ...
        clear: function () {
            this.data = [];
            this.isInit = false;
            $(document).off('click', this.wrap + ' .fun-tabs-btn')
        },
        change: function (route, callback) {
            ...
            if (existsTab) {
                ...
                layui.fun.navigate(route.href)
            } else {
                self.loadHtml(fileurl, function (res) {
                    var htmlElem = $(
                        "<div><div class='fun-tabs-item' lay-url='" +
                        fileurl +
                        "'>" +
                        res.html +
                        '</div></div>'
                    ...
                    tab.data.push(route);
                    layui.fun.render(tab.tabMenuTplId);
            ...
            layui.fun.sidebarFocus(route.href);
            return false
        },
        onChange: function () {
        }
